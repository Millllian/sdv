...
class Plant(pygame.sprite.Sprite):
    def __init__(self, plant_type, groups, soil, check_watered):
        ...
        self.frames = import_folder(f's4 - Animations/graphics/fruit/{plant_type}')
        self.soil = soil
...
class soilLayer:
    def __init__(self, all_sprites, collision_sprites):
        ...
        #graphics
        self.soil_surf = pygame.image.load('s4 - Animations/graphics/soil/o.png').convert_alpha()
        self.soil_surfs = import_folder_dict('s4 - Animations/graphics/soil')
        self.water_surfs = list(import_folder_dict('s4 - Animations/graphics/soil_water').values())

        self.create_soil_grid()
        self.create_hit_rects()

    def create_soil_grid(self):
        ground = pygame.image.load('s4 - Animations/graphics/world/ground.png').convert_alpha()
        h_tiles = ground.get_width() // TILE_SIZE
        v_tiles = ground.get_height() // TILE_SIZE

        self.grid = [[[] for  col in range(h_tiles)] for row in range(v_tiles)] 
        for x, y, _  in load_pygame('s4 - Animations/data/map.tmx').get_layer_by_name('Farmable').tiles():
        ...
        for index_row, row in enumerate(self.grid):
                    for index_col,cell in enumerate(row):
                        if 'X' in cell and 'W' not in cell:
                            ...
                            x = index_col * TILE_SIZE
                            y = index_row * TILE_SIZE
                            WaterTile((x,y), choice(self.water_surfs), groups=(self.all_sprites, self.water_sprites))
